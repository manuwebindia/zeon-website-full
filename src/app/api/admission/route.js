...
export async function POST(request) {
  try {
    ...
    const { full_name, email, phone, recaptchaToken } = body;
    ...
    if (!recaptchaData.success || recaptchaData.score < 0.5 || recaptchaData.action !== 'admission_form') {
...
<!-- Notice the class="body" added here! This is required for the Gmail hack -->
<body class="body" style="margin: 0; padding: 0; background-color: #f4f4f5;">
  <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;border:1px solid #e5e7eb;border-radius:12px;overflow:hidden;background-color:#ffffff;">
    ...
    <!-- HEADER FIX: CSS Classes applied + slightly offset gradient (#2d2e2e to #2d2e2f) -->
    <div class="locked-bg" style="background-color:#2d2e2e; background-image:linear-gradient(to bottom, #2d2e2e, #2d2e2f); padding:24px 32px;">
      ...
      <h2 class="locked-text-white" style="margin:0; font-size:1.4rem; color:#ffffff;">New Consultation Request</h2>
      <p class="locked-text-pink" style="margin:6px 0 0; font-size:0.9rem; color:#ffc7c7;">Via Zeon Academy Website Landing Page</p>
    </div>
    
    <!-- BODY (Left alone so it adapts nicely to dark/light mode) -->
    <div style="padding:32px;background-color:#ffffff;">
      <table style="width:100%;border-collapse:collapse;">
        ...
        <tr>
          <td style="padding:10px 0;color:#6b7280;font-size:0.9rem;">Phone</td>
    ...
    <div style="background-color:#f9fafb;padding:16px 32px;text-align:center;">
      <p style="color:#9ca3af;font-size:0.8rem;margin:0;">Zeon Academy · Admission Enquiry System</p>
    </div>
    ...
    console.error("[admission] Mail error:", error);
